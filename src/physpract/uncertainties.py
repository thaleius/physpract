  ...
  def short(self, unit: bool = True, predecimals: int = 1) -> str:
    ...
    if abs(correct) >= 1:
      ...
      if rounded_uncertainty == 0:
        ...
        s = f"{to_non_scientific_string(rounded_value.scaleb(-correct))}({bracket_notation(rounded_value, rounded_uncertainty, correct)})" + (f"e{correct}" if correct != 0 else "")
    else:
      if rounded_uncertainty == 0:
        s = f"{value}"
      else:
        s = f"{value}({bracket_notation(rounded_value, rounded_uncertainty)})"
